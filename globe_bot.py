...
r = praw.Reddit(username = 'globe_bot',
            ...
            user_agent = "<platform>:<app ID>:<version string> (by /u/<reddit username>) globe comment responder v0.1")
...
for submission in r.subreddit('test').new(limit=20):
    ...
    for comment in post.comments.list():
        ...
        for word in comment_string:
            #if country name in country list matches a word in the comment continue
            if word in get_countries:
                ...
                print(country_code)
                #start Geocoding API
                gmaps = googlemaps.Client(key='AIzaSyBWeTlyImUJXWqtbou7pRw5BdSnSyvPy4A')
                ...
                map_url = "https://www.google.com/maps/search/?api=1&query="+latitude+","+longitude+"&query_place_id="+country_id
